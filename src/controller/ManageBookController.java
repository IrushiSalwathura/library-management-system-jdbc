...
public class ManageBookController {
    ...
    public void btnSave_OnAction(ActionEvent actionEvent) {
        ...
        if(btnSave.getText().equals("SAVE BOOK")) {
            try {
                PreparedStatement pstm = DBConnection.getInstance().geConnection().prepareStatement("INSERT INTO Book VALUES (?,?,?,?)");
            ...
            try {
                PreparedStatement pstm = DBConnection.getInstance().geConnection().prepareStatement("UPDATE Book SET isbn=?,title=?,author=?,edition=? WHERE isbn ='"+isbn+"'");
                pstm.setObject(1, isbn);
                pstm.setObject(2, title);
                pstm.setObject(3, author);
                pstm.setObject(4,edition);
    ...
    public void imgDelete_OnAction(MouseEvent mouseEvent) {
        String isbn = txtIsbn.getText();
        try {
            PreparedStatement pstm = DBConnection.getInstance().geConnection().prepareStatement("DELETE  FROM Book WHERE isbn=?");
            pstm.setObject(1,isbn);
            int affected = pstm.executeUpdate();
            if(affected>0){
