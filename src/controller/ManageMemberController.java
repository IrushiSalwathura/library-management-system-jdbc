...
public class ManageMemberController {
    ...
    public void btnSave_OnAction(ActionEvent actionEvent) {
        ...
        if(btnSave.getText().equals("SAVE MEMBER")) {
            try {
                PreparedStatement pstm = DBConnection.getInstance().geConnection().prepareStatement("INSERT INTO Member VALUES (?,?,?)");
            ...
            try {
                PreparedStatement pstm = DBConnection.getInstance().geConnection().prepareStatement("UPDATE Member SET nic=?,name=?,address=? WHERE nic ='"+nic+"'");
                pstm.setObject(1, nic);
                pstm.setObject(2, name);
                pstm.setObject(3, address);
                int affected = pstm.executeUpdate();
    ...
    public void imgDelete_OnMouseClicked(MouseEvent mouseEvent) {
        String nic = txtMemberID.getText();
        try {
            PreparedStatement pstm = DBConnection.getInstance().geConnection().prepareStatement("DELETE  FROM Member WHERE nic=?");
            pstm.setObject(1,nic);
            int affected = pstm.executeUpdate();
            if(affected>0){
